# @rankbox/plugin-starter

The shared React + Vite shell every Rankbox CMS plugin forks. It handles the
parts that are identical across Framer / Webflow / Shopify:

- API-key entry + validation (via `ping`)
- Listing the account's finished articles (via `@rankbox/api-client`)
...
Set the API base URL to your app (`https://rankbox.top`) and paste a key from
